...
[V, Kv, Mv] = fem_CraigBampton(15 * 6, K, M, bc_nodes, bc_dofs);
...
[V, Kv, Mv] = fem_CraigBampton(16 * 6, K, M, bc_nodes, bc_dofs);
...
n_modes = 12;
[V, Kv, Mv] = fem_CraigBampton(n_modes, K, M, bc_nodes, bc_dofs);
assert(all(size(V) == [17 * 6, 12 + n_modes]))
% Static modes are placed properly
assert(norm(V(1:6, 1:6) - eye(6)) == 0)
assert(norm(V(end-5:end, 7:12) - eye(6)) == 0)
assert(all(size(Kv) == [12 + n_modes, 12 + n_modes]))
assert(all(size(Mv) == [12 + n_modes, 12 + n_modes]))
