% Script to make basic test for point contraint
...
s0 = [9; 2; -1];
sys = h_sys_constr_point(sys, s0, q0, 1, 2);

assert(sys.nconstr == 3);
assert(length(sys.constr.point) == 1);
...
sys = h_sys_constr_point(sys, s0, q0, 4, 3);
sys = h_sys_constr_point(sys, s0, q0, 2, 4);
sys = h_sys_constr_point(sys, rand(3, 1), q0, 1, 2); % For rigid bodies this can be anywhere!
